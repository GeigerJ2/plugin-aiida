function __aiida.init
    ...
    # verdi status => vst
    __aiida.create_abbr vst 'verdi status'
    ...
    function __aiida.add_tide_prompt -d "Add scientist logo and AiiDA profile to the right side of the Tide prompt."
        set filepath "$HOME/.config/fish/fish_variables"

        # Check if the file exists
        if test -f $filepath
            if not grep -q "tide_aiida_bg_color" $filepath
                echo "SETUVAR tide_aiida_bg_color:444444" >> $filepath
                echo "SETUVAR tide_aiida_color:FF872A" >> $filepath
                echo "SETUVAR tide_aiida_icon:\U000f0320" >> $filepath
                # sed -i 's|\\x1epython\\x1ejava|\\x1epython\\x1eaiida\\x1ejava|' $filepath -> Doesn't work
            end
        end
    end

    __aiida.add_tide_prompt


    # Cleanup declared functions
    functions -e __aiida.create_abbr # __aiida.add_tide_prompt
end
